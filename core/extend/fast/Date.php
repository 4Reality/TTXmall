...
class Date
{
     ...
     * @return mixed
     */
    public static  function inWeekTime($time,  $day = 7) {
        ...
        for ($i = 0;$i < $day ; $i++){
            $data['seven_week'][$i] = strtotime( '+'.$i+1-$week .' days',$newtime);
            $data['seven_week_staend'][$i] = self::showStartEndTime($data['seven_week'][$i]);
        }
        return $data;
    }
     ...
     *
     * */
    public static function showStartEndTime($time)
    {
        $data['start'] = mktime(0, 0, 0, date('m' , $time), date('d' , $time), date('Y' , $time));
        $data['end'] = mktime(23, 59, 59, date('m' , $time), date('d' , $time), date('Y' , $time));
        return $data;
    }
}
